...
class Avanza extends EventEmitter {
   ...
   * @param {String} [options.price] The target price to receive an alert on.
   * @param {String} [options.validUntil] A date on the form YYYY-MM-DD. Cancels alert if this date is passed.
   */
  addPriceAlert(instrumentId, options) {
    options.notification = true
    const path = constants.paths.PRICE_ALERT_PATH.replace('{0}', instrumentId)
