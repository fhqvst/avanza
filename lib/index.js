...
class Avanza extends EventEmitter {
  ...
  * @param {String} credentials.totp_secret
  */
  authenticate(credentials) {
    ...
    return new Promise((resolve, reject) => {
      ...
      request({
        ...
        const totpCode = (credentials.totp_secret) ? totp(credentials.totp_secret) : credentials.totp

        if (!totpCode) {
          return Promise.reject('Missing credentials.totp or credentials.totp_secret')
        }
