...
class Avanza extends EventEmitter {
  ...
  */
  authenticate(credentials) {
    ...
    return new Promise((resolve, reject) => {
      ...
      request({
        ...
        this._scheduleReauth(credentials, (this._authenticationTimeout - 1) * 60 * 1000)
   ...
   */
  _scheduleReauth(credentials, delay) {
    clearTimeout(this._reauthentication)
    this._reauthentication = setTimeout(() => {
      this.authenticate(credentials)
          .catch((error) => {
            this._scheduleReauth(credentials, this._backoffCalc('authenticate'))
          })
    }, delay)
  }
