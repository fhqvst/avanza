...
class Avanza extends EventEmitter {
  ...
  _socketInit(restart) {
    ...
    this._socket.on('close', () => {
      this._socketConnected = false
      setTimeout(() => { this._socketInit(true); }, _backoffCalc('websocket'))
    })
  }
  ...
  */
  authenticate(credentials) {
    ...
    if (!(this._authenticationTimeout >= MIN_INACTIVE_MINUTES &&
          this._authenticationTimeout <= MAX_INACTIVE_MINUTES)) {
      return Promise.reject('Session timeout not in range ' +
        MIN_INACTIVE_MINUTES + '-' + MAX_INACTIVE_MINUTES + ' minutes.')
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      request({
        ...
        }, (this._authenticationTimeout - 1) * 60 * 1000)
