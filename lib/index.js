...
class Avanza extends EventEmitter {
  ...
  *
  * @param {String} accountOrTransactionType A valid account ID or a [Transaction Type](#transaction-type).
  ...
  */
   ...
   *
   * @param {String} instrumentType Instrument type of the pertaining instrument. See [Instrument Types](#instrument-types).
   * @param {String} orderbookId The orderbookId of the instrument.
  ...
  */
  deleteOrder(accountId, orderId) {
    const path = constants.paths.ORDER_PLACE_DELETE_PATH + '?' + querystring.stringify({
      accountId: accountId,
      orderId: orderId
    })
